...
export function monthRate(ratePerHour, discount) {
    let monthrate;
    if (discount != 0) {
        monthrate = (dayRate(ratePerHour) * 22) * (1 - discount);
    } else {
        monthrate = dayRate(ratePerHour) * 22;
    }
    return Math.ceil(monthrate);
}
...
export function daysInBudget(budget, ratePerHour, discount) {
    return Math.floor(budget / (Math.floor(dayRate(ratePerHour)) * (1 - discount)));
}
